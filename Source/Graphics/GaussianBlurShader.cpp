	...
	gaussianFilterData)
{
	...
	cbFilter.kernelSize = (float)kernelSize;
	...
	for (int y =- kernelSize / 2; y <= kernelSize / 2; y++)
	{
		for (int x = -kernelSize / 2; x <= kernelSize / 2; x++)
		{
			...
			cbFilter.weights[id].z = (float)exp(-(x * x + y * y) / (deviationPow2) )
            / (DirectX::XM_PI * deviationPow2);

			//cbFilter.weights[id].z = (float)1 / (DirectX::XM_PI * 2.0f * deviationPow) * exp(
			//	-(cbFilter.weights[id].x * cbFilter.weights[id].x
			//		+ cbFilter.weights[id].y * cbFilter.weights[id].y)
			//	/ (deviationPow));
