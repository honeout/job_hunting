...
AfterImageShader::AfterImageShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\AfterImageShaderVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\AfterImageShaderPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	// 定数バッファ
	{
		...
		// アルファ値用バッファ
		desc.ByteWidth = sizeof(CbAlphaset);

		hr = device->CreateBuffer(&desc, 0, alphasetConstantBuffer.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
...
// 描画開始
void AfterImageShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
...
// 描画
void AfterImageShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void AfterImageShader::End(ID3D11DeviceContext* dc)
{
