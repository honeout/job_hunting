...
void FinalpassShader::Begin(const RenderContext& rc)
{
	...
	ID3D11Buffer* constatBuffers[] =
	{
		finalpassConstatBuffer.Get(),
	};
	rc.deviceContext->VSSetConstantBuffers(0, ARRAYSIZE(constatBuffers), constatBuffers);
	rc.deviceContext->PSSetConstantBuffers(0, ARRAYSIZE(constatBuffers), constatBuffers);
}
