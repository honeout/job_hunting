...
#include "Projectile\ProjectileHoming.h"
#include "Projectile\ProjectileFullHoming.h"
#include "Projectile\ProjectileStraight.h"
...
void WanderState::Execute(float elapsedTime)
{
		...
		animationTime - FLT_EPSILON <= 4.1f + FLT_EPSILON)
	{
		...
		{
			// 足音
			paramSe.filename = AudioConfig::audioLanding;
...
void DamageState::Enter()
{
	...
	// 衝撃波音
		// 斬撃音
	paramSe.filename = AudioConfig::audioCoveredDamage;
	Audio::Instance().Play(paramSe);
...
void PlayerMovestate::Enter()
{
	...
	// se足音再生
	paramSe.filename = AudioConfig::audioMove;
	Audio::Instance().Play(paramSe);
...
void PlayerQuickJabState::Execute(float elapsedTime)
{
	...
	if (!rotateCheck)
	{
		...
		if (length < PlayerConfig::attackCheckRange && !isEnemyHit)
		{
			...
			{
				// Seテレポート再生
				paramSe.filename = AudioConfig::audioTelleport;
				Audio::Instance().Play(paramSe);
				// エフェクト
		...
		else
		{
			...
			if (!moveid->GetOnLadius())
			{
				...
				modelAnimUpperBody.currentAnimationAddSeconds = slashAnimSpeed;
				modelAnim.loop = true;
				//modelAnim.currentAnimationAddSeconds = 0.7f;
				// アニメーション再生
				model->PlayAnimation(modelAnim);
			...
			else
			{
				...
				modelAnim.currentAnimationAddSeconds = slashAnimSpeed;
				// アニメーション再生
				model->PlayAnimation(modelAnim);
				// 攻撃三回で一度強制終了
				++attackMemory;
...
void PlayerCycloneStrikeState::Enter()
{
	...
	if (!moveid->GetOnLadius())
	{
		...
		modelAnimUpper.currentAnimationAddSeconds = slashAnimSpeed;
		// アニメーション上半身再生
...
void PlayerSpecialAttackState::Enter()
{
	...
	// 雷音
	paramSe.filename = AudioConfig::audioThunder;
	Audio::Instance().Play(paramSe);
...
void PlayerSpecialAttackState::Execute(float elapsedTime)
{
	...
	else
	{
		...
		if (animationTime >= 1.6f)
		{
			...
			// se雷再生
			paramSe.filename = AudioConfig::audioThunderSpecial;
			Audio::Instance().Play(paramSe);
...
void PlayerMagicState::Execute(float elapsedTime)
{
		...
		playerid->InputMagick(elapsedTime))
	{
		...
		// 魔法チャージ等オフ
		playerid->SetIsMagicChageEnd(false);
	}
...
// 回転
	...
	case (int)Player::CommandMagic::Fire:
	{
		...
		// 火音
		paramSe.filename = AudioConfig::audioFire;
		Audio::Instance().Play(paramSe);
	...
	case (int)Player::CommandMagic::Ice:
	{
		...
		// se氷再生
		paramSe.filename = AudioConfig::audioIce;
		Audio::Instance().Play(paramSe);
	...
	case (int)Player::CommandMagic::Heale:
	{
		...
		// se回復再生
		paramSe.filename = AudioConfig::audioHeale;
		Audio::Instance().Play(paramSe);
		// 回復発動
...
void PlayerSpecialMagicState::Execute(float elapsedTime)
{
	...
	else
	{
		...
		if (animationTime >= 1.1f && animationTime <= 1.2f)
		{
			...
			// se炎再生
			paramSe.filename = AudioConfig::audioFireMove;
			Audio::Instance().Play(paramSe);
		}
	}
	// ダメージ判定
...
void PlayerAvoidanceState::Enter()
{
	...
	AudioParam paramSe;
	// ダッシュse再生
	paramSe.filename = AudioConfig::audioDush;
	Audio::Instance().Play(paramSe);
...
void PlayerAvoidanceState::Execute(float elapsedTime)
{
	...
	if (animationTime >= 0.7f && animationTime <= 0.8f)
	{
		...
		{
			dir.x * (speed * elapsedTime),
			0,
			dir.z * (speed * elapsedTime),
		};
